...
const handleGenerateQuestions = async (params: {
	...
	const chunkPrompt = chunks
		.map(
			(chunk) => `Chunk ${chunk.id} (${chunk.label}):\n"""\n${chunk.text}\n"""`,
		)
		.join("\n\n");
	...
	const questions: ElaborationQuestion[] = parsed.questions
		...
		.map((item, index) => {
			...
			const difficulty =
				item.difficulty === "advanced" ? "advanced" : "standard";
			const modelAnswer = (item.modelAnswer ?? "").trim();
			const focus = item.focus?.trim() ?? undefined;
			const refs = Array.isArray(item.chunkRefs)
				? item.chunkRefs.slice(0, 2)
				: [];
...
const handleEvaluateAnswer = async (params: {
	...
	const context = question.anchors
		.map(
			(anchor) => `Chunk ${anchor.chunkId} (${anchor.label}):\n${anchor.text}`,
		)
		.join("\n\n");
	...
	const highlights: EvaluationHighlights = {
		correctIdeas: Array.isArray(parsed.highlights?.correctIdeas)
			? parsed
					.highlights!.correctIdeas.map((item) => String(item).trim())
					.filter((item) => item.length > 0)
					.slice(0, 4)
			: [],
		missingLinks: Array.isArray(parsed.highlights?.missingLinks)
			? parsed
					.highlights!.missingLinks.map((item) => String(item).trim())
					.filter((item) => item.length > 0)
					.slice(0, 4)
			: [],
		suggestions: Array.isArray(parsed.highlights?.suggestions)
			? parsed
					.highlights!.suggestions.map((item) => String(item).trim())
					.filter((item) => item.length > 0)
					.slice(0, 4)
			: [],
	};

	const modelAnswer = parsed.modelAnswer?.trim()?.length
		? parsed.modelAnswer.trim()
		: question.modelAnswer;
	const followUp = parsed.followUp?.trim()?.length
...
const handleSessionSummary = async (params: {
	lovableApiKey: string;
	documentTitle: string;
	evaluations: SummarizeInput[];
}) => {
	...
	const conceptsExplainedWell = Array.isArray(parsed.conceptsExplainedWell)
		? parsed.conceptsExplainedWell
				.map((item) => String(item).trim())
				.filter((item) => item.length > 0)
				.slice(0, 6)
		: [];

	const conceptsNeedingElaboration = Array.isArray(
		parsed.conceptsNeedingElaboration,
	)
		? parsed.conceptsNeedingElaboration
				.map((item) => String(item).trim())
				.filter((item) => item.length > 0)
	...
	try {
		...
		if (action === "evaluate-answer") {
			...
			if (
				...
				!Array.isArray(question.anchors)
			) {
				return json(400, { error: "question payload is invalid" });
			}

			const data = await handleEvaluateAnswer({
				lovableApiKey,
				question,
		...
		if (action === "summarize-session") {
			...
			const data = await handleSessionSummary({
				lovableApiKey,
				documentTitle,
				evaluations,
			});
