		...
		.map((kp, index) => ({
			...
			importance:
				typeof kp.importance === "number"
					? Math.min(Math.max(kp.importance, 0), 1)
					: 0.5,
		}))
...
const handleExampleSummary = async (
	...
	const wordCount =
		summaryWords.length > 0 ? summaryWords.length : (parsed.wordCount ?? 0);
...
const handleEvaluate = async (
	...
	const coverage = {
		...
		missed: Array.isArray(parsed.coverage.missed)
			? parsed.coverage.missed.map((item, index) => ({
					...
					reason:
						item.reason?.trim() || "Consider mentioning this idea explicitly.",
				}))
			: [],
	};
	...
	return {
		...
		strengths: Array.isArray(parsed.strengths)
			? parsed.strengths.map((item) => item.trim()).filter(Boolean)
			: [],
		improvements: Array.isArray(parsed.improvements)
			? parsed.improvements.map((item) => item.trim()).filter(Boolean)
			: [],
		rewriteHints: Array.isArray(parsed.rewriteHints)
			? parsed.rewriteHints.map((item) => item.trim()).filter(Boolean)
			: [],
	};
};
...
serve(async (req) => {
	...
	if (
		!actionValue ||
		!["key-points", "example-summary", "evaluate-summary"].includes(actionValue)
	) {
	...
	try {
		...
		if (actionValue === "key-points") {
			const result = await handleKeyPoints(
				lovableApiKey,
				document.title,
				normalized,
				wordTarget,
		...
		if (actionValue === "example-summary") {
			const result = await handleExampleSummary(
				lovableApiKey,
				document.title,
				normalized,
				wordTarget,
